...
Images inside post content should go through `next/image`, not a raw `<img>` tag, even inside MDX. This is one of the easier things to miss, since it's tempting to just write `![alt text](/image.png)` in Markdown and move on. A custom MDX component that maps the Markdown image syntax to `next/image` under the hood keeps the lazy-loading and responsive sizing benefits without changing how anyone writes content.
